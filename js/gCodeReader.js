...
GCODE.gCodeReader = (function() {
    ...
    var purgeLayers = function() {
        // remove all layers with no extrusions
        model = model.filter(function(x) { for(var j in x) if(x[j].extrude) return true; return false; })
    };
    ...
    return {
        ...
        xloadFile: function(reader) {
            ...
            GCODE.ui.worker.postMessage({ cmd: "parseGCode", msg: { gcode: lines } });
            delete lines;
        },
